...
class Reg_user extends CI_Model
{
    ...
    public function get_mis_requerimientos_pagina($idcom, $pagina)
    {
        ...
        $query = $this->db->query(
            "SELECT * FROM requerimientos, comercios, oportunidades_negocio, estatus_req, usuarios WHERE  oportunidades_negocio.opneg_id=estatus_req.opnegocio_id AND requerimientos.req_id=oportunidades_negocio.requerimiento_id AND requerimientos.usuario_id=comercios.usuario_id AND comercios.usuario_id=usuarios.usuario_id  AND estatus_req.requerimiento_id = requerimientos.req_id AND oportunidades_negocio.comercio_id='" .
                $idcom .
