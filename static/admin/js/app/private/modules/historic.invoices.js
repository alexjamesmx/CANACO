jQuery( document ).ready( ( $ ) => {
  ...
  $( document ).on( 'blur', '.update-tc-date', function ( event ) {
    ...
    if ( dateValue.length === 10 ) {
      $.ajax( {
        url: base_url( 'app/historicinvoices/get_historic_exchange/' ),
        type: 'POST',
        dataType: 'json',
        data: {
  ...
  $( document ).on( 'click', '#btn-save-modal-commentinvoice', function ( event ) {
    ...
    $.ajax( {
      url: base_url( 'app/historicinvoices/add_note' ),
      type: 'post',
      dataType: 'json',
      data: {
        folio,
  ...
  $( document ).on( 'change', '.invoice-cancelstate', function ( event ) {
    ...
    $.ajax( {
      url: `${base_url()}app/historicinvoices/cancelstate/`,
      type: 'post',
      dataType: 'json',
      data: {
        id: elem.attr( 'data-f' ),
  ...
  $( document ).on( 'click', '.save-invoice', function ( event ) {
    ...
    $.ajax( {
      url: `${base_url()}app/historicinvoices/updateinvoice/`,
      type: 'post',
      dataType: 'json',
      data: {
        IdFac,
  ...
  $( document ).on( 'click', '.btn-add-complement', () => {
    ...
    $.ajax( {
      url: `${base_url()}app/historicinvoices/addcomplement/`,
      type: 'post',
      dataType: 'json',
      data: {
        IdFac,
  ...
  $( document ).on( 'click', '.btn-add-creditnote', () => {
    ...
    $.ajax( {
      url: `${base_url()}app/historicinvoices/addcreditnote/`,
      type: 'post',
      dataType: 'json',
      data: {
        IdFac,
...
function view_file( folio, type ) {
  ...
  $( '#iframe-modal-view-file-invoice' ).attr(
    'src',
    base_url( `app/historicinvoices/viewfile/${folio}/${type}` ),
  )
